...
const ConvertDueDate = ({ dueDate }) => {
	const [theDue, setTheDue] = useState(new Date(dueDate));
	...
	useEffect(() => {
		setTheDue(new Date(dueDate));
	}, [dueDate]);

	const day = dateFormat(theDue, 'dddd, mmmm dS, yyyy');
	const time = dateFormat(theDue, 'hh:MM:ss TT');
	return (
...
const Todo = ({ todo, setTodos, changeDisplay, setError }) => {
			...
			{!changeDisplay ? (
				<label
					...
					<ul className="list-group list-group-flush">
						...
						<ConvertDueDate dueDate={dueDate} />
					</ul>
